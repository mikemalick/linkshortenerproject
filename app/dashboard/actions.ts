...
export async function createLinkAction(input: {
  ...
  try {
    ...
    return {
      error: "Failed to create link. That short code may already be taken.",
    };
  }
...
export async function updateLinkAction(
  ...
  try {
    await updateLink(id, userId, {
      originalUrl: parsed.data.originalUrl,
      shortCode: parsed.data.shortCode,
    });
  } catch {
    return {
      error: "Failed to update link. That short code may already be taken.",
    };
  }
